from sqlalchemy import String, DateTime, Integer, ForeignKey, LargeBinary, Table, Column
from sqlalchemy.orm import Mapped, mapped_column, relationship
from datetime import datetime, timezone
from typing import List
from infrastructure.db.session import Base

# --- טבלאות קשר (מחוץ לקלאסים) ---
# ניהול יחסי אמון בין משתמשים (Many-to-Many)
...
# --- המודל המקורי שלך (ללא שינוי בעמודות הקיימות) ---
class UserModel(Base):
    ...
    # הוספת קשרים (Relationships) כדי שה-Repositories יעבדו חלק
    files: Mapped[List["FileModel"]] = relationship("FileModel", back_populates="owner")

# --- המודלים החדשים שנדרשים למשימה שלך ---
...
class FileModel(Base):
    ...
    versions: Mapped[List["FileVersionModel"]] = relationship("FileVersionModel", back_populates="file", cascade="all, delete-orphan")
